...
CURLcode Curl_http_readwrite_headers_zc(struct Curl_easy *data,
                                     struct connectdata *conn,
                                     ssize_t *nread,
                                     bool *stop_reading)
{
    ...
    do {
      ...
      if((conn->proxy_negotiate_state == GSS_AUTHDONE) &&
        ...
        conn->proxy_negotiate_state = GSS_AUTHSUCC;
            ...
            headerlen = Curl_dyn_len(&data->state.headerb);
            result = Curl_client_write_zc(data, writetype,
                                       Curl_dyn_ptr(&data->state.headerb),
                                       headerlen);
            if(result)
                return result;
...
#define HEADER1 headp /* no conversion needed, just use headp */
        ...
        result = Curl_client_write_zc(data, writetype, headp,
                                   Curl_dyn_len(&data->state.headerb));
        if(result)
            return result;
