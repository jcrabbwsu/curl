...
CHUNKcode Curl_httpchunk_read_zc(struct Curl_easy *data,
                              char *datap,
                              ssize_t datalen,
                              ssize_t *wrotep,
                              CURLcode *extrap)
{
    ...
    *wrote = 0; /* nothing's written yet */
    ...
    /* the original data is written to the client, but we go on with the
       chunk read process, to properly calculate the content length*/
    if(data->set.http_te_skip && !k->ignorebody) {
        result = Curl_client_write_zc(data, CLIENTWRITE_BODY, datap, datalen);
        if(result) {
            *extrap = result;
            return CHUNKE_PASSTHRU_ERROR;
        }
        ...
        switch(ch->state) {
            ...
            case CHUNK_DATA:
                ...
                */
                ...
                if(!data->set.http_te_skip && !k->ignorebody) {
                    if(!data->set.http_ce_skip && k->writer_stack)
                        result = Curl_unencode_write_zc(data, k->writer_stack, datap, piece);
                    else
                        result = Curl_client_write_zc(data, CLIENTWRITE_BODY, datap, piece);
            ...
            case CHUNK_TRAILER:
                if((*datap == 0x0d) || (*datap == 0x0a)) {
                    ...
                    if(tr) {
                        ...
                        if(!data->set.http_te_skip) {
                            result = Curl_client_write_zc(data, CLIENTWRITE_HEADER, tr, trlen);
                            if(result) {
                                *extrap = result;
                                return CHUNKE_PASSTHRU_ERROR;
                            }
